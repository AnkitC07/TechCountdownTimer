...
async function uiExtensionInit({ name, extensionType, app, extensionFlavor, extensionDirectory, }) {
    const list = ui.newListr([
        ...
        },
        {
            ...
            task: async (_, task) => {
                ...
                const templateDirectory = await path.findUp(`templates/ui-extensions/projects/${mapExtensionTypeToExternalExtensionType(extensionType)}`, {
                    type: 'directory',
...
export function getRuntimeDependencies({ extensionType, extensionFlavor, }) {
...
async function changeIndexFileExtension(extensionDirectory, extensionFlavor) {
    ...
    if (fileExtension) {
        await file.move(path.join(extensionDirectory, 'src/index'), path.join(extensionDirectory, `src/index.${fileExtension}`));
    }
}
